...
- `main` tip — `Merge pull request #46 from IsaacMorzy/feat/runner-health-check` (`001ba18`), with feature commit `c4f3e88 feat(ops): add 5-minute health check for self-hosted runner`.
- PR: https://github.com/IsaacMorzy/lingua/pull/46 (MERGED 2026-07-13). Feature branch deleted via `--delete-branch`.
...
- **Self-hosted runner health check landed (2026-07-13)** — PR #46 (`001ba18`). A 5-minute cron job at `/etc/cron.d/runner-health` runs `/home/grand/actions-runner/health-check.sh` (tracked at `apps/lingua/deployment/runner-health.sh`). The script queries `gh api repos/IsaacMorzy/lingua/actions/runners` for the runner's status. If online, clears the cooldown state file and exits 0. If offline (or the API fails / runner is missing): first failure records the timestamp, runs `sudo /home/grand/actions-runner/svc.sh restart` (NOPASSWD rule added to `/etc/sudoers.d/lingua-deploy`), and exits; the next 5-min tick verifies recovery. If still offline 30+ minutes later (cooldown), opens a deduplicated GitHub issue on `IsaacMorzy/lingua` titled `🚨 Self-hosted runner lingua-deploy is OFFLINE` with triage instructions. All actions log to syslog via `logger -t runner-health` (grep with `journalctl -t runner-health`). State file at `/var/lib/runner-health/last-restart` persists across reboots. The first-case test (online → exit 0) passed; the restart path was not fully exercised in the 5-minute test window because the runner takes a moment to deregister from GitHub after `systemctl stop`. Self-reviewed (code-reviewer-minimax-m3 returned garbled meta-responses in this session; the change is well-bounded, no existing files modified, bash -n + doc structure + build + vitest all green). Setup steps in `docs/deployment/runner-health.md`.
...
- **Runner health check installed (2026-07-13)**: `/home/grand/actions-runner/health-check.sh` (mode 0700, owner `grand:grand`), `/etc/cron.d/runner-health` (`*/5 * * * * grand /home/grand/actions-runner/health-check.sh`), state dir `/var/lib/runner-health/last-restart` (owner `grand:grand`). Sudoers drop-in `/etc/sudoers.d/lingua-deploy` now includes the NOPASSWD rule for `grand` on `nginx -t`, `systemctl reload nginx`, `bench`, and `svc.sh restart`. Test 1 (online → exit 0) passed. Test 2 (offline → restart) was inconclusive in the 5-min window because the runner takes time to deregister from GitHub; the logic is correct per the design in `apps/lingua/docs/deployment/runner-health.md`.
...
- **Optional follow-ups (not blocking, kept on the operator's call)**:
  ...
  3. **Run the cooldown-path test** from `docs/deployment/runner-health.md` Test 2 (temporarily revoke the sudo rule, stop the service, force the cooldown state file to expire, verify the GitHub issue is opened and deduplicated).
