...
[
  {
    "date": "2026-07-13",
    "focus": "Bench migrate gate end-to-end green (PR #53) + sudoers hardening (PR #52) + audit",
    "actions": [
      "Followups from the prior turn: (1) audit the deployed sudoers rules against the workflow's sudo calls; (2) extend the sudoers drop-in to also cover `svc.sh start` + `svc.sh stop` so manual recovery from the runner user doesn't need operator password; (3) test the bench-migrate gate end-to-end via `workflow_dispatch` with `migrate=true`.",
      "Audit (read-only, 5 min): grepped the workflow for every `sudo` call (4 calls: `nginx -t`, `systemctl reload nginx`, `svc.sh restart`, `bench --site <site> migrate`). All 4 have exact NOPASSWD rules in the drop-in. 0 rules in the drop-in are unused. 0 hard-coded service names in the workflow that bypass `svc.sh`.",
      "Sudoers hardening applied server-side: /etc/sudoers.d/lingua-deploy grew from 4 to 6 NOPASSWD rules. The two new rules: `grand ALL=(ALL) NOPASSWD: /home/grand/actions-runner/svc.sh start` and `... stop`. `visudo -c` parsed OK. `sudo -n svc.sh start` works (returns non-zero only because the script must be invoked from `/home/grand/actions-runner`, not because of sudo). Runner is still online.",
      "docs/deployment/secrets.md updated to reflect the 6-rule drop-in + the recovery-incident rationale + the service-name-discovery pattern. Branch `chore/sudoers-start-stop`, commit, push, open PR #52. Code-reviewer verdict: `safe to merge` (no HARD, no SOFT). Merged. New main tip: `ddfd8df`.",
      "Gate test #1: triggered `gh workflow run deploy-frontend.yml --ref main -f migrate=true` (run #29217083726). The run 'succeeded' but the migrate step concluded as `Skipped` — bug #1 discovered. The deploy step itself worked because it only copies static files; the site name was never tested.",
      "Bug investigation: ran `bench --site ijlaps.ac.ke migrate` on the server → `Site ijlaps.ac.ke does not exist!` — bug #2 discovered. The public domain (`ijlaps.ac.ke`, the nginx `server_name` and `site_config.json` `host_name`) is NOT the Frappe site directory name (`kenyaschooloflanguages.ac.ke`). Three distinct 'site names' exist on this server, a trap for future operators. Also confirmed the `if` condition bug: `inputs.migrate == 'true'` compares a boolean to a string in GitHub Actions expressions (no coercion), so it's always false. The step is correctly gated — too correctly, it never runs.",
      "Gate test #2 (fix attempt): rewrote the workflow `if` to `inputs.migrate == true` (boolean comparison) and renamed the site to `kenyaschooloflanguages.ac.ke` in 3 workflow places + 2 secrets.md places + server-side sudoers drop-in. Branch `fix/gate-site-name-and-boolean`, commit, push, open PR #53 (first commit). Code-reviewer verdict: `safe to merge`.",
      "Operator correction during the PR #53 review: the public domain is `ijlaps.ac.ke` (per the nginx `server_name` and the `site_config.json` `host_name`), NOT `morzy.kenyaschooloflanguages.ac.ke` (which is a redirect/legacy hostname). Added a second commit to PR #53 fixing the two stale `morzy.kenyaschooloflanguages.ac.ke` references in secrets.md (Sudo configuration section + the comment block) and the workflow migrate-step comment, plus a missed `ijlaps.ac.ke` reference in the secrets.md Security model bullet's sudoers rule. Code-reviewer verdict: `safe to merge`. PR #53 merged (two commits). New main tip: `5518956`.",
      "Gate test #3 (verification): re-triggered `gh workflow run deploy-frontend.yml --ref main -f migrate=true` (run #29217342342). **Completed `success`**. Build → copy → nginx -t → reload nginx → migrate (with `MIGRATE_INPUT: true`) → `bench --site kenyaschooloflanguages.ac.ke migrate` → exit 0. Site reachable at `https://ijlaps.ac.ke/` (HTTP 200). Runner is online. **The bench-migrate gate is production-ready.**",
      "STATE.md and loop-run-log.md updated: main tip pointer to `5518956`, Recent decisions gained entries for the gate end-to-end green (PR #53) and the sudoers hardening (PR #52), Active work follow-ups updated to mark the recovery-incident hardening as DONE, Caveats runner-health installed note updated to list the 6 NOPASSWD rules. loop-run-log.md gained this entry + matching markdown section."
    ],
    "outcome": "PR #52 (sudoers hardening) and PR #53 (gate fix: boolean condition + site name) both merged. The bench-migrate gate is now production-ready and end-to-end green: run #29217342342 completed `success` with the migrate step running `bench --site kenyaschooloflanguages.ac.ke migrate` and exiting 0. The audit confirmed all 4 workflow sudo calls have exact NOPASSWD rules and 0 rules in the drop-in are unused. Sudoers extended from 4 to 6 rules (added `svc.sh start` + `svc.sh stop` for passwordless manual recovery). Two bugs in the original PR #49 were caught and fixed: the `if` condition compared a boolean to a string (always false), and the site name was the public domain instead of the Frappe site directory name. Three distinct 'site names' exist on this server (public domain `ijlaps.ac.ke`, nginx `server_name` `ijlaps.ac.ke`, `site_config.json` `host_name` `https://ijlaps.ac.ke`, Frappe site directory `kenyaschooloflanguages.ac.ke`) — a trap documented in the workflow + secrets.md comments. Deploy pipeline is green and monitored end-to-end."
  },
  {
    "date": "2026-07-13",
...
## 2026-07-13 — Bench migrate gate end-to-end green (PR #53) + sudoers hardening (PR #52)

- **Audit (2026-07-13)**: grepped `.github/workflows/deploy-frontend.yml` for every `sudo` call (4 calls: `nginx -t`, `systemctl reload nginx`, `svc.sh restart`, `bench --site <site> migrate`). All 4 have exact NOPASSWD rules in `/etc/sudoers.d/lingua-deploy`. 0 rules in the drop-in are unused.
- **Sudoers hardening landed via PR #52 (2026-07-13)** — the drop-in grew from 4 to 6 NOPASSWD rules. The two new rules grant `grand` passwordless sudo on `/home/grand/actions-runner/svc.sh start` and `... stop`, addressing the 2026-07-13 recovery incident where the runner went offline and `sudo systemctl start` required a password from the runner user. `svc.sh start` / `svc.sh stop` are the runner-bundled helpers that resolve the right systemd unit name internally; now NOPASSWD. `visudo -c` parsed OK. `docs/deployment/secrets.md` updated to reflect the 6-rule drop-in + the recovery-incident rationale + the service-name-discovery pattern. Code-reviewer verdict: `safe to merge` (no HARD, no SOFT).
- **Gate test discovered two bugs in PR #49 (2026-07-13)**: triggered `gh workflow run deploy-frontend.yml --ref main -f migrate=true` (run #29217083726). The run "succeeded" but the migrate step concluded as `Skipped` — bug #1: the `if` condition `inputs.migrate == 'true'` compared a boolean to a string and was always false. The deploy step itself worked because it only copies static files (`apps/lingua/lingua/{www,public/frontend}`); the static file handler serves them without touching the Frappe site. So the site name was never tested. A subsequent `bench --site ijlaps.ac.ke migrate` on the server revealed bug #2: `Site ijlaps.ac.ke does not exist!` — the public domain (`ijlaps.ac.ke`, the nginx `server_name` and `site_config.json` `host_name`) is NOT the Frappe site directory name (`kenyaschooloflanguages.ac.ke`). Three distinct "site names" exist on this server, a trap for future operators.
- **Gate test fixed via PR #53 (2026-07-13)** — two commits on `fix/gate-site-name-and-boolean`. First commit: `if: github.event_name == 'workflow_dispatch' && inputs.migrate == true` (boolean comparison) + site name → `kenyaschooloflanguages.ac.ke` in 3 workflow places + 2 secrets.md places + server-side sudoers drop-in. Second commit (operator correction): fixed two stale `morzy.kenyaschooloflanguages.ac.ke` references in secrets.md (it's a redirect/legacy hostname, not the public domain) and the Security model bullet that still referenced `ijlaps.ac.ke` in the sudoers rule context. visudo parsed OK; `sudo -n bench --site kenyaschooloflanguages.ac.ke migrate` works; `sudo -n bench --site ijlaps.ac.ke migrate` now prompts. Code-reviewer verdict: `safe to merge` (no HARD, no SOFT) for both commits.
- **Gate test PASSED (2026-07-13)** — run #29217342342 (re-triggered after PR #53 merge) completed `success`. Build → copy → nginx -t → reload nginx → migrate (with `MIGRATE_INPUT: true`) → `bench --site kenyaschooloflanguages.ac.ke migrate` → exit 0. Site reachable at `https://ijlaps.ac.ke/` (HTTP 200). Runner is online. Main is at `5518956`. **The bench-migrate gate is production-ready.**
- **STATE.md and loop-run-log.md updated** to record the full chain: audit → sudoers hardening → gate test discovery → fix → green run. The Recent decisions section gained entries for the gate end-to-end green (PR #53) and the sudoers hardening (PR #52). The Active work follow-ups section was updated to mark the recovery-incident hardening as DONE. Caveats runner-health installed note updated to list the 6 NOPASSWD rules.
